...
setting is a deployed product against a live database, which lets us report a failure class, verification, that a
simulated tool environment cannot produce at all.
...
whose rejection messages are fixed and form part of the benchmark. We hold the model fixed and vary the harness,
which is the axis those designs hold constant by construction.
...
We would encourage reporting the rejection-message text and the parser's tolerance alongside results, as
benchmark artefacts in their own right.
